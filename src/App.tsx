  ...
  return (
    <div className="min-h-screen flex bg-white text-slate-700 font-sans selection:bg-slate-200">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-xl flex items-center justify-between">
                      <div>
                        <p className="text-slate-400 text-sm uppercase font-bold tracking-wider mb-1">Total Programs</p>
                        <p className="text-5xl font-black tracking-tight text-white"><NumberTicker value={stats.totalPrograms} /></p>
                      </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
                      <div>
                        <p className="text-slate-400 text-sm uppercase font-bold tracking-wider mb-1">Monthly Programs</p>
                        <p className="text-4xl font-black tracking-tight text-indigo-600"><NumberTicker value={stats.monthlyCount} /></p>
                      </div>
                           ...
                           return (
                             <div key={editor.id}>
                               <div className="flex justify-between text-xs mb-1">
                                 <span className="font-bold text-slate-600">{editor.name}</span>
                                 <span className="font-mono text-slate-500">{count}</span>
                               </div>
                               <div className="h-2.5 bg-slate-50 rounded-full overflow-hidden">
                                 <AnimatedBar width={max === 0 ? 0 : (count / max) * 100} colorClass={theme.bar} />
                               </div>
                             </div>
                           )
                        })}
        ...
        {view === 'calendar' && <button onClick={() => { setEditingTask({}); setIsTaskModalOpen(true); }} className="absolute bottom-8 right-8 bg-slate-800 hover:bg-slate-900 text-white p-4 rounded-full shadow-lg hover:shadow-xl transition-all transform hover:scale-105 active:scale-95 z-50"><Plus size={24} strokeWidth={2.5} /></button>}
      </main>
            ...
            <div className="p-5 space-y-4">
               {editingTask.id && editingTask.startDate && editingTask.endDate && (
                <div className={`p-2 rounded-md text-xs font-bold flex items-center gap-2 ${getTaskStatus(editingTask) === 'completed' ? 'bg-indigo-50 text-indigo-600' : getTaskStatus(editingTask) === 'active' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
                  ...
                  {getTaskStatus(editingTask) === 'completed' ? '已完成' : getTaskStatus(editingTask) === 'active' ? '進行中' : '未開始'}
                </div>
              )}
              <div className="space-y-1"><label className="block text-[11px] font-bold text-slate-500 uppercase">節目</label><select className="w-full p-2 bg-white border border-slate-200 rounded-md text-sm" value={editingTask.show || ''} onChange={e => setEditingTask({...editingTask, show: e.target.value})}><option value="" disabled>選擇節目...</option>{programs.map(p => <option key={p.id} value={p.name}>{p.name} ({p.duration})</option>)}</select></div>
              <div className="grid grid-cols-5 gap-3"><div className="col-span-2 space-y-1"><label className="block text-[11px] font-bold text-slate-500 uppercase">集數</label><input type="text" className="w-full p-2 border border-slate-200 rounded-md text-sm" value={editingTask.episode || ''} onChange={e => setEditingTask({...editingTask, episode: e.target.value})} /></div><div className="col-span-3 space-y-1"><label className="block text-[11px] font-bold text-slate-500 uppercase">剪輯師</label><select className="w-full p-2 bg-white border border-slate-200 rounded-md text-sm" value={editingTask.editor || ''} onChange={e => setEditingTask({...editingTask, editor: e.target.value})}><option value="" disabled>選擇...</option>{displayEditors.map(e => <option key={e.name} value={e.name}>{e.name}</option>)}</select></div></div>
              <div className="grid grid-cols-2 gap-3"><div className="space-y-1"><label className="block text-[11px] font-bold text-slate-500 uppercase">開始</label><input type="date" className="w-full p-2 border border-slate-200 rounded-md text-sm" value={editingTask.startDate || ''} onChange={e => setEditingTask({...editingTask, startDate: e.target.value})} /></div><div className="space-y-1"><label className="block text-[11px] font-bold text-slate-500 uppercase">結束</label><input type="date" className="w-full p-2 border border-slate-200 rounded-md text-sm" value={editingTask.endDate || ''} onChange={e => setEditingTask({...editingTask, endDate: e.target.value})} /></div></div>
              <div className="space-y-1"><label className="block text-[11px] font-bold text-slate-500 uppercase">備註 (Note)</label><textarea className="w-full p-2 border border-slate-200 rounded-md text-sm h-20 resize-none outline-none focus:border-slate-400 transition" placeholder="輸入備註..." value={editingTask.note || ''} onChange={e => setEditingTask({...editingTask, note: e.target.value})} /></div>
            </div>
            <div className="px-5 py-3 bg-slate-50 border-t border-slate-100 flex justify-between">{editingTask.id ? <button onClick={() => deleteTask(editingTask.id!)} className="text-red-500 hover:bg-red-50 px-2 rounded"><Trash2 size={16}/></button> : <div/>} <div className="flex gap-2"><button onClick={() => setIsTaskModalOpen(false)} className="px-4 py-1.5 text-sm font-bold text-slate-500">Cancel</button><button onClick={saveTask} className="bg-slate-800 text-white px-4 py-1.5 rounded-md text-sm font-bold shadow-sm">Save</button></div></div>
          </div>
        </div>
      )}

      {/* Program & Editor Modals */}
